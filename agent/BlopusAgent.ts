...
async function boot(): Promise<void> {
  ...
  const _opp = (personalityProfile?.voiceProfile as any)?.originalPostProfile
  const _oppEmojiRule = _opp?.emojiContext
    ? `emoji: ${_opp.emojiContext}`
    : (_opp?.emojiFrequency > 0) ? `emoji in ${_opp.emojiFrequency}% of posts` : 'no emojis'
  const engagementEngine = isOwnerMode ? new EngagementEngine(
    ...
    _opp?.synthesized ?? personalityProfile?.voiceProfile?.synthesized,
  ...
  async function runCycle(): Promise<void> {
    ...
    for (const decision of prioritized) {
      ...
      // Per-post cap — limit total replies Blopus sends to commenters on a single post
      // Uses conversationId (= root tweet ID on Twitter) to count all replies in that thread
      const maxPerPost = replyBackRules?.maxRepliesPerPost
      if (maxPerPost != null && isOwnerMode) {
        const rootId = decision.mention.threadContext?.rootTweet?.id ?? decision.mention.conversationId
        if (rootId) {
          const postReplyCount = memory.getConversationReplyCount(rootId)
          if (postReplyCount >= maxPerPost) {
            log('info', `Skipping @${decision.mention.authorHandle} — per-post cap reached (${postReplyCount}/${maxPerPost}) on post ${rootId}`)
            continue
          }
        }
      }
      ...
      // Fall back to threadContext rootTweet when the post wasn't recorded in memory
      // (e.g. owner manually posted it — not generated by Blopus)
      const rootTweet = decision.mention.threadContext?.rootTweet
      const ownPostContext = originalPost
        ...
        : (rootTweet?.text && isOwnerMode)
          ? `This person is replying to YOUR tweet: "${rootTweet.text.slice(0, 280)}". Respond knowing what you said.`
          : undefined
